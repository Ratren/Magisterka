...
static void gen_float_arr(float* arr, size_t n) {
    for (size_t i = 0; i < n; i++) arr[i] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
}

static double calc_max_error(const float* Y1, const float* Y2, size_t n) {
    double e = 0.0;
    for (size_t i = 0; i < n; i++) {
        double d = fabs((double)Y1[i] - (double)Y2[i]);
        if (d > e) e = d;
    }
    return e;
}
...
static int measure_one(Impl* impl, int Cin, int H, int W, int K, int Cout,
    ...
    if (t_one > MAX_SECONDS_PER_CALL) {
        impl->median = impl->min = impl->max = impl->stddev = 0.0;
        impl->max_error = calc_max_error(Y, Y_ref, ySize);
        return 0;
    }
...
static void print_summary(const PresetResult* results, int n) {
    if (n <= 1) return;
    printf("\n================================================================\n");
    printf("              PODSUMOWANIE - Median GFLOPS (max %d runs)\n", NUM_RUNS);
    printf("================================================================\n");
