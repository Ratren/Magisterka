...
\begin{enumerate}
  \item Dense matrix factorizations such as Householder QR and Hessenberg reduction\cite{golub2013}.
  ...
  \item Recurrent neural network (RNN/LSTM) inference\cite{grnn}, where each step performs dense matrix--vector multiplications with the weight matrices.
\end{enumerate}
...
\item \textbf{DRAM:} DDR4-3200 dual-channel provides
      ...
      (Figure~\ref{fig:bandwidth}), but GEMV's highly regular streaming
...
\paragraph{Bandwidth efficiency across cache levels.}
...
\paragraph{Cross-implementation comparison.}
...
\paragraph{Roofline model validation.}
...
\bibitem{grnn}
Holmes, C., Mawhirter, D., He, Y., Yan, F., Wu, B.:
GRNN: Low-Latency and Scalable RNN Inference on GPUs.
In: Proceedings of the Fourteenth EuroSys Conference (EuroSys'19),
pp.~41:1--41:16. ACM, New York (2019).
