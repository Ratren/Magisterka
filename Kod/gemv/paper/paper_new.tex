...
We present a hand-optimized GEMV kernel for AMD Zen~3, implemented using
AVX2 (Advanced Vector Extensions~2) and FMA (fused multiply-add) intrinsics. Our single-threaded kernel (ZenGEMV) processes 4~rows simultaneously
...
\item A source-level analysis identifying the concrete cause of lost GEMV
      performance on Zen~3---most notably OpenBLAS's reuse of a Haswell
      microkernel that supplies only 4 of the 8 independent FMA chains the
      dual-FMA pipeline requires---together with a roofline account of where
      each library's throughput is bound.
\end{enumerate}
...
The operation requires $2MN$ floating-point operations---one multiplication and one addition
...
\item \textbf{DRAM:} DDR4-3200 dual-channel provides
      ...
      (Section~\ref{sec:results}) approaches the full 51.2\,GB/s
      ceiling, which we therefore adopt as an approximate practical DRAM
      bandwidth.
\end{itemize}
...
Cache- and vectorization-aware optimization of dense linear algebra has also
been studied at FedCSIS, with attention to energy consumption: Bylina et~al.\
examine how loop transformations and storage order affect both the performance
and energy of dense matrix factorization on multicore x86\cite{bylina2022wz},
while Jakobs and R\"unger measure the performance and energy cost of the AVX
load/store variants that dominate memory-bound kernels\cite{jakobs2018avx}.
...
\begin{figure}[tbp]
\begin{minted}[fontsize=\scriptsize,frame=single,linenos,numbersep=4pt,xleftmargin=2em]{c}
for (; j + 15 < cols; j += 16) {
...
\begin{itemize}
\item \textbf{AOCL-BLAS on medium (1T):} AOCL-BLAS achieves 23.72~GFLOPS vs.\ our
      22.92~GFLOPS on $1024 \times 1024$. The AOCL-BLAS Zen~3 kernel configuration
...
Since this marginally exceeds the theoretical figure, the result should be
read as the kernel saturating the DDR4-3200 memory controller to within
measurement tolerance, with residual L3 reuse across the 5~timed iterations
(the $4096\times4096$ matrix at 128\,MB exceeds the 32\,MB L3, yet a fraction
remains resident between iterations) a plausible source of the small excess.
...
(89.7\% vs.\ 86.7\%).
AOCL-BLAS's low 65.8\% on the wide preset ($256\times8192$) stems from its
transpose (DOTXF) code path (\texttt{bli\_gemv\_unf\_var1}), which fuses
\texttt{BLIS\_DF}$=8$ dot products and so traverses 8~columns concurrently while
reusing~$\mathbf{x}$ (of length~$N$). At $N=8192$, $\mathbf{x}$ (64\,KB)
overflows L1---the only L3-resident preset for which it does---and the eight
64\,KB-strided streams are served less efficiently than ZenGEMV's four-stream,
row-major pattern, which sustains 86.6\% at the same shape.
...
\textbf{Application-level impact:} The present evaluation isolates GEMV at the
kernel level; measuring the kernel within a complete application where GEMV
dominates runtime would quantify how kernel-level gains translate into overall
speedups.
...
shape-adaptive dispatch for non-square matrices, extended support for
arbitrary memory layouts and scaling parameters, and an evaluation of the kernel
within a complete application where GEMV dominates runtime.
...
\url{https://doi.org/10.48550/arXiv.2502.04563}
...
\textit{Int. J. Database Theory Appl.,} vol.~9, no.~2, pp.~47--60, 2016.
\url{https://doi.org/10.14257/ijdta.2016.9.2.06}

\bibitem{singh2021gemv}
A. Singh and C. Bassoy,
``High-Performance Level-1 and Level-2 BLAS,''
arXiv:2108.02025 [cs.MS], 2021.
\url{https://doi.org/10.48550/arXiv.2108.02025}

\bibitem{bylina2022wz}
B. Bylina, J. Bylina, and M. Piekarz,
``Influence of loop transformations on performance and energy consumption of the multithreaded WZ factorization,''
in \textit{Proc. 17th Conf. Computer Science and Intelligence Systems (FedCSIS),
Ann. Comput. Sci. Inf. Syst.,} vol.~30, pp.~479--488, 2022.
\url{https://doi.org/10.15439/2022F251}

\bibitem{jakobs2018avx}
T. Jakobs and G. R\"unger,
``On the energy consumption of Load/Store AVX instructions,''
in \textit{Proc. 2018 Federated Conf. Computer Science and Information Systems (FedCSIS),
Ann. Comput. Sci. Inf. Syst.,} vol.~15, pp.~319--327, 2018.
\url{https://doi.org/10.15439/2018F28}
